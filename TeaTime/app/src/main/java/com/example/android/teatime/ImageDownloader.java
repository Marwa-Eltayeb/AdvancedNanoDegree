...
class ImageDownloader {
    ...
    interface DelayerCallback {
        // It is called inside the run method
    ...
    static void downloadImage(Context context, final DelayerCallback callback,
        ...
        if (idlingResource != null) {
            // If it is not idle, setIdleState to false and do something in background THREAD
        ...
        handler.postDelayed(new Runnable() {
            ...
            public void run() {
                if (callback != null) {
                    // Put ArrayList data here
                    callback.onDone(mTeas);
                    if (idlingResource != null) {
                        // Now it is idle, so you can test
